...
<%
		...
		//System.out.println("trdd sql-->"+sql);	 	
		List<Map<String, Object>> list = xdbc.queryForList(sql);
		//System.out.println("trdd list size-->-->"+list.size());
 		...
 		+"' and XMLTYPE='ASS' AND ZT = '0' and FY like '"+JSUtils.fjm.substring(0,1)+"%' GROUP BY SUBSTRING(FY,1,2)";
		//System.out.println("trdd sql-->"+sql);	 	
		...
		Map<String,Object> dmap2 = new HashMap<String,Object>();
	 	...
	 	for(int i=0;i<fy_List.size();i++){
			...
			//if(dmap.containsKey(map.get("DM"))){
				jo.put(map.get("DM")+"_trdd",dmap.get(map.get("DM"))==null?0:dmap.get(map.get("DM")));
				jo.put(map.get("DM")+"_trdd_sb",dmap2.get(map.get("DM"))==null?0:dmap2.get(map.get("DM")));
			//}
		}
