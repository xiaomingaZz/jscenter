...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		initTree([{'contentid':'ywcbContent','treeid':'ywcbTree','inputid':'ywcb','selected':['<%=fydm%>'],'expand':true,'hide':false,
			'valueid':'ywcbValue','onClick':ywcbClick,'url':contextPath+'/webapp/lljk/fc/lljk_fc_tree.jsp?'+param+"&FYDM="+fydm}]);
		sizeChange(true);
		loadTable(1,true);
	});
	...
	function loadTable(page,first){
		...
		loadData('./lljk_fc_data.jsp?'+param,{'start':page,'limit':limit,'FYDM':fydm},function(json){
			$("#tableContent").unmask();
	...
	function showXml(ah,xmlname,sbyy){
		
		window.open("lljk_xml.jsp?xmlname="+xmlname+"&ah="+encodeURI(encodeURI(ah))+"&sbyy="+encodeURI(encodeURI(sbyy)));
	}
</script>
</html>