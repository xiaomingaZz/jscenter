...
String jssj = StringUtils.trim(request.getParameter("jssj"));
	...
	String param = "gz="+gz+"&kssj="+kssj+"&jssj="+jssj;
%>
<html>
<head>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		initTree([{'contentid':'ywcbContent','treeid':'ywcbTree','inputid':'ywcb','selected':['<%=fydm%>'],'expand':true,'hide':false,
			'valueid':'ywcbValue','onClick':ywcbClick,'url':contextPath+'/webapp/spxt/sjzl/fc/sjzl_fc_tree.jsp?'+param+"&FYDM="+fydm}]);
		sizeChange(true);
		loadTable(1,true);
	});
	...
	function loadTable(page,first){
		...
		loadData('./sjzl_fc_data.jsp?'+param,{'start':page,'limit':limit,'FYDM':fydm},function(json){
			$("#tableContent").unmask();
	...
	function doFc1(ahdm){
		window.open('xmlfile.jsp?ahdm='+ahdm);
	}
</script>
</html>