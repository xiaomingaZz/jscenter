...
<%
	...
	if("jc".equals(dl)){
		sql += " "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' ) "
			+  " AND AJZT >='300' AND LARQ < '"+kssj+"' and (AJZT < '800' OR AJZT >='800' AND JARQ >='"+kssj+"') "
			+  " GROUP BY FYDM";
	}else if("sa".equals(dl)){
		sql += " "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' AND LARQ > '') "
			+  " AND AJZT >='300' AND LARQ >='"+kssj+"' and LARQ <='"+jssj+"' "
			+  " GROUP BY FYDM";
	}else if("ja".equals(dl)){
		sql += " "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='800' AND JARQ > '' ) "
		+  " AND AJZT >='800' AND JARQ >='"+kssj+"' and JARQ <='"+jssj+"' "
		+  " GROUP BY FYDM";
	}else{
		sql += " "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' ) "
		+  " AND AJZT >='300' AND LARQ <= '"+jssj+"' and (AJZT < '800' OR AJZT >='800' AND JARQ > '"+jssj+"') "
		+  " GROUP BY FYDM";
	}
