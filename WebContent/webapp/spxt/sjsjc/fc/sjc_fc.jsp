...
String jssj = StringUtils.trim(request.getParameter("jssj"));
	...
	String param = "flag="+flag+"&kssj="+kssj+"&jssj="+jssj;
%>
<html>
<head>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		initTree([{'contentid':'ywcbContent','treeid':'ywcbTree','inputid':'ywcb','selected':['<%=fydm%>'],'expand':true,'hide':false,
			'valueid':'ywcbValue','onClick':ywcbClick,'url':contextPath+'/webapp/spxt/sjsjc/fc/sjc_fc_tree.jsp?'+param+"&FYDM="+fydm}]);
		sizeChange(true);
		loadTable(1,true);
	});
	...
	function loadTable(page,first){
		...
		loadData('./sjc_fc_data.jsp?'+param,{'start':page,'limit':limit,'FYDM':fydm},function(json){
			$("#tableContent").unmask();
	...
	function doExport(){
		...
		_params += "&FYDM="+fydm;
		document.getElementById('exportFrame').src = "<%=CONTEXT_PATH%>/webapp/spxt/sjsjc/fc/sjc_fc_export.jsp?"+_params;
	}
</script>
</html>