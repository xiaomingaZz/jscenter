	...
	try{
		...
		if("jc".equals(dl)){
			sql += " AND AJZT >='300' AND LARQ < '"+kssj+"' AND (AJZT < '800' or AJZT >='800' AND JARQ >='"+kssj+"')"
				+" AND "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' AND LARQ < '"+kssj+"' and (AJZT < '800' OR AJZT >='800' AND JARQ >='"+kssj+"')) ";
		}else if("sa".equals(dl)){
			sql += " AND AJZT >='300' AND LARQ >= '"+kssj+"' AND LARQ <='"+jssj+"' "
				+" AND "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' AND LARQ >='"+kssj+"' and LARQ <='"+jssj+"') ";
		}else if("ja".equals(dl)){
			sql += " AND AJZT >='800' AND JARQ >= '"+kssj+"' AND JARQ <='"+jssj+"' "
				+" AND "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='800' AND JARQ >='"+kssj+"' and JARQ <='"+jssj+"') ";
		}else{
			sql += " AND AJZT >='300' AND LARQ <= '"+jssj+"' AND (AJZT < '800' or AJZT >='800' AND JARQ > '"+jssj+"')"
				+" AND "+column+" NOT IN (SELECT "+column2+" FROM "+table2+" WHERE AJZT >='300' AND LARQ <= '"+jssj+"' and (AJZT < '800' OR AJZT >='800' AND JARQ > '"+jssj+"')) ";
		}
