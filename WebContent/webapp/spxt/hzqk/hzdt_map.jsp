...
<%
	...
	if(dm.length() == 2 || dm.length() == 4 && dm.endsWith("00")){
		...
		sql = "SELECT substring(ID_YWCB,1,2) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       ...
		       +" FROM  FACT_SJC "
		       +" WHERE ID_MTH>='"+kssj+"' and ID_MTH<= '"+jssj+"' and  ID_YWCB like '"+fjm.substring(0,1)+"%'  GROUP BY substring(ID_YWCB,1,2) ";
	}else if(dm.length() == 4){
		...
		sql = "SELECT substring(ID_YWCB,1,3) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       +" SUM(CASE ID_MTH when '"+jssj+"' then WJS else 0 end) as WJS"
		       +" FROM  FACT_SJC "
		       +" WHERE ID_MTH>='"+kssj+"' and ID_MTH<= '"+jssj+"' and  ID_YWCB like '"+fjm+"%'  GROUP BY substring(ID_YWCB,1,3) ";
	}
