...
<%
		...
		String sql = "select ID_AJXZ DM,SUBSTRING(ID_AJLX,4,1) DM2,"+column+" NUM from FACT_SJC_AJXZ "
			+" where "+ cond+" and ID_YWCB like '"+fjm+"%' group by ID_AJXZ,SUBSTRING(ID_AJLX,4,1) ";
		//System.out.println("hzdt pie1 sql-->"+sql);
	 	...
	 	for(int i = 0 ; i < series.size() ; i++){
	 		...
	 		//	+"<br>---------<br>一审:"+ys+"件<br>二审:"+es+"件<br>再审:"+zs+"件'},";
	 		x = x + "'" + mm.get("name")+"',";
	 	}
	 	x = x.substring(0,x.length() - 1);
 %>
option = {
     title : {show:true,x:'center',text: '<%=mc %>案件类型<%=name %>组成情况(<%=total %>)',
	     textStyle:{
		    fontSize: 26,
		    fontWeight: 'bolder',
		    color: '#000000',
