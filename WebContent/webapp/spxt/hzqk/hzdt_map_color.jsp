...
<%
	...
	String where = " where ID_MTH >= '"+kssj+"' AND ID_MTH <= '"+jssj+"' ";
	...
	if(dm.length() == 2 || dm.length() == 4 && dm.endsWith("00")){
		...
		sql = "SELECT substring(ID_YWCB,1,2) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       +" SUM(CASE ID_MTH when '"+jssj+"' then WJS else 0 end) as WJS"
		       +" FROM  FACT_SJC "
		       +" WHERE ID_MTH>='"+kssj+"' and ID_MTH<= '"+jssj+"' and  ID_YWCB like '"+fjm.substring(0,1)+"%'  "
		       +cond
		       +" GROUP BY substring(ID_YWCB,1,2) ";
		sql_tq = "SELECT substring(ID_YWCB,1,2) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       +" SUM(CASE ID_MTH when '"+jssj+"' then WJS else 0 end) as WJS"
		       +" FROM  FACT_SJC "
		       +" WHERE ID_MTH>='"+kssj2+"' and ID_MTH<= '"+jssj2+"' and  ID_YWCB like '"+fjm.substring(0,1)+"%'  "
		       +cond
		       +" GROUP BY substring(ID_YWCB,1,2) ";
	}else if(dm.length() == 4){
		fylist = bi09.queryForList("select FJM DM,NAME_CITY from DC_CITY where DATALENGTH(FJM) = 3 AND FJM LIKE '"+fjm+"%' order by FJM ");
		sql = "SELECT substring(ID_YWCB,1,3) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       +" SUM(CASE ID_MTH when '"+jssj+"' then WJS else 0 end) as WJS"
		       +" FROM  FACT_SJC "
		       +" WHERE ID_MTH>='"+kssj+"' and ID_MTH<= '"+jssj+"' and  ID_YWCB like '"+fjm+"%'  "
		       +cond
		       +" GROUP BY substring(ID_YWCB,1,3) ";
		sql_tq = "SELECT substring(ID_YWCB,1,3) DM,SUM(SAS) AS SAS,SUM(JAS) AS JAS, "
		       +" SUM(CASE ID_MTH when '"+jssj+"' then WJS else 0 end) as WJS"
		       +" FROM  FACT_SJC "
